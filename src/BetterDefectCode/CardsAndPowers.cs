...
public sealed class BdStaticDischargeChargePower : PowerModel
{
    private sealed record ChargeBatch(int EligibleFromPlaySerial, decimal Bonus);
    private readonly List<ChargeBatch> _batches = new();

    public override PowerType Type => PowerType.Buff;
    public override PowerStackType StackType => PowerStackType.Counter;

    internal void RecordNewCharge(decimal bonus)
    {
        if (bonus <= 0m) return;
        // A Lightning generated by an Attack must not be consumed by or boost
        // that same card. It becomes available starting with the next card
        // play; Skill/Power-generated Lightning follows the same rule.
        _batches.Add(new ChargeBatch(CurrentPlaySerial() + 1, bonus));
    }
...
#endif
    {
        ...
        var serial = CurrentPlaySerial();
        var trackedTotal = _batches.Sum(batch => batch.Bonus);
        if (_batches.Count == 0 || trackedTotal < Amount)
            return Amount; // Save/rewind compatibility fallback.
        return _batches.Where(batch => batch.EligibleFromPlaySerial <= serial).Sum(batch => batch.Bonus);
    }
    ...
    public override async Task AfterCardPlayed(PlayerChoiceContext context, CardPlay cardPlay)
    {
        ...
        var serial = CurrentPlaySerial();
        var consumed = _batches
            .Where(batch => batch.EligibleFromPlaySerial <= serial)
            .Sum(batch => batch.Bonus);
        if (_batches.Count == 0 && Amount > 0m)
            consumed = Amount;
        if (consumed <= 0m)
            return;

        _batches.RemoveAll(batch => batch.EligibleFromPlaySerial <= serial);
        Flash();
        await Bd.ModifyPowerAmount(context, this, -consumed, Owner, cardPlay.Card);
    }

    private static int CurrentPlaySerial()
    {
