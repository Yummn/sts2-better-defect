...
- [v0.6.13](https://github.com/Yummn/sts2-better-defect/releases/tag/v0.6.13): restores the card-library disable/enable UI on mobile, fixes Chinese dynamic-odds text, restores the disabled-card grey mask, and shows a compact top segmented disabled-card counter HUD. Verified on Android v103 card compendium via ADB screenshots.
...
- The top segmented disabled-card counter shows 0-25 in blue and 26-35 in red; v0.6.13 moves it to the very top so it does not cover card names.
- `Data/Portraits/*.png` is included for CardBeautify's restored old Defect art.
- No BaseLib dependency.

## Install

Download `BetterDefect-v0.6.13.zip` from Releases, unzip it, and copy the `BetterDefect` folder into the game's `mods` folder.
